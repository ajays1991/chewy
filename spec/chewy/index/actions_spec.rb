...
describe Chewy::Index::Actions do
  ...
  describe '.reindex' do
    ...
    context 'reindexing a index should delete the old index and point the alias to the new index' do
      ...
      before { City.create!(id: 1, name: 'Moscow') }
      before { City.create!(id: 2, name: 'San Fransisco') }
      before { CitiesIndex.create '2013' }
      let(:city) { CitiesIndex::City }
      before { city.import }
      specify { expect(Chewy.client.indices.exists(index: 'cities')).to eq(true) }
      specify { expect(Chewy.client.indices.exists(index: 'cities_2013')).to eq(true) }
      specify { expect(CitiesIndex.aliases).to eq([]) }
      specify do
        CitiesIndex.reindex('2015')
        expect(Chewy.client.indices.exists(index: 'cities_2015')).to eq(true)
